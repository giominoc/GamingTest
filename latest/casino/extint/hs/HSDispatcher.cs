...
namespace GamingTests.Latest.Casino.ExtInt.HS
{
    public sealed class HSDispatcher : ICasinoExtIntFaceContract
    {
        private readonly IWalletPipelineContract _pipeline;

        public HSDispatcher(IWalletSharedHelpersContract helpers)
        {
            _pipeline = new HSWalletCore(helpers);
        }
        ...
        public Task<WalletResult> GetAuxInfosAsync(string action, IDictionary<string, object> payload, CancellationToken cancellationToken = default)
        {
            ...
            {
                "bet" => _pipeline.ExecuteAsync(WalletOperation.Bet, request, cancellationToken),
                "win" => _pipeline.ExecuteAsync(WalletOperation.Win, request, cancellationToken),
        ...
        private static WalletRequest Map(IDictionary<string, object> payload)
        {
            return new WalletRequest
            {
                ...
                GameRound = payload.TryGetValue("roundRef", out var round) ? (string)round : string.Empty,
