...
{
    public sealed class HSWalletRules : IProviderWalletRules
    {
        public string Provider => "HS";
        ...
        public async Task<WalletResult> ExecuteWalletAsync(WalletContext context, IWalletSharedHelpersContract helpers, CancellationToken cancellationToken = default)
        {
            ...
            return WalletResult.Ok(balance, $"HS-{request.TransferId}");
        }
    }

    public sealed class HSWalletCore : BaseProviderWalletCore
    {
        public HSWalletCore(IWalletSharedHelpersContract helpers) : base(new HSWalletRules(), helpers)
        {
        }
    }
}
