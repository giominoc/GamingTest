...
{
    public class CasinoAMSwDispatch : FinMovExtWsFace
    {
        ...
        public static string AMSWFACEKEY
        {
            get
            {
                if(!CasinoExtIntMgr.def.checkSignedFace(_AMSWFACEKEY))
                    CasinoExtIntMgr.def.registerSignedFace(_AMSWFACEKEY, CasinoExtIntAMSWCore.def);
                return _AMSWFACEKEY;
            }
        }

        public static CasinoAMSwDispatch _def;
        public static bool defReload;

        public static CasinoAMSwDispatch def
        {
            get
            {
                if (_def == null || defReload)
                {
                    _def = (CasinoAMSwDispatch)CCFactory.Get(typeof(CasinoAMSwDispatch));
                    defReload = false;
                }
                return _def;
            }
